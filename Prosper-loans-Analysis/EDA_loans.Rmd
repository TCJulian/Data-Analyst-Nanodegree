# Exploratory Data Analysis of Prosperity Loans
_by Tyler Julian_
...
In the finance, everyone is looking at getting the best deal. Best interest rate, best return, low risk and high reward... as both borrowers and lenders, we want it all. This primarily takes form in the interest rate of a loan. Unfortunately, what borrowers want and what lenders want do not always line up. Even worse is trying to understand the factors that determine the interest rate. For borrowers, credit score is often the most used variable, but what other factors impact the loans? The amount of the loan? Where you live? How much income you have? And what about lenders? How much risk is involved in a "High Risk" account? Does return really fluctuate that much between a low risk and high risk account? Are there any factors that predict when an account will default? This project hopes to explore some of these issues and reveal the biggest factors that influence both lenders and borrowers.
...
The dataset can be downloaded from this project's GitHub repository: [prosperLoanData.csv](https://github.com/TCJulian/Data-Analyst-Nanodegree/blob/master/Prosper-loans-Analysis/prosperLoanData.csv)
...
There are a multitude variables in this dataset, some of whose meaning may not be intuitive at first glance. This [reference](https://docs.google.com/spreadsheets/d/1gDyi_L4UvIrLTEC6Wri5nbaMmkGmLQBk-Yx3z0XDEtI/edit#gid=0), adapted from the Prosper API, provides context to each of the variables. 

## Univariate Analysis

In this section, potential variables of interest will be explored for  

### Borrower Interest Rate

One of most important variables for both a borrower and a lender is the interest rate of the loan. The interest rate is ultimately set by Prosperity and is decided based upon by a variety of variables.
...
```{r, echo=FALSE, warning=FALSE, AvgCreditScore_Histogram}
...
Looking at only the unique values of `Term`, the only terms offered by Propser are 12, 36, and 60 term lengths. The API shows that this variable is measured in months. 
...
In fact, over 99% of all loans through Prosperity have a positive estimated return. From a lender standpoint, this is excellent news. However, porjections do not always live up to reality.
...
California appears to have much more borrowers than any other state. This is understandable sense [Propser was founded in San Francisco California](https://www.prosper.com/plp/about/contact-us/). The states following California--Texas, New York, Florida--are also metro hubs with large tech industries.

### Credit Grade and Prosper Rating

In an effort to make investments easier to assess for potential investors, Prosper created the CreditGrade scoringsystem. This was eventually replaced with the Prosper Rating system in 2009, which is very similiar to CreditGrade. In order to make an analyze these variables throughout all of the data, these two variables need to be combined into one. 
...
Currenlty, there is a lot of detail in the levels. However, the granularity of the data can be reduces by combining some of the levels. More specifically, some of the `Past Due` bins can be combined. `FinalPaymentInProgress` loans can also be lumped with `Current` loans.
...
Loan amount is the total amount of cash given to the borrower once the loan is approved. Loans start at \$1000 and go up to a maximum of \$35,000.
...
Another common metric requested from lenders is the borrower's current employment status. Employment status gives the lender a gauge on the potential cashflow the borrower currently has. More cashflow usually represetns a safer loan.

```{r, echo=FALSE, EmploymentStatus_levels}
levels(loans$EmploymentStatus)
```

Currently, the levels are not ordered and are listed nominally. `Not available` and `""` basically represent the same idea aand can be combined.

To better analyse the variable, the levels will be porperly ordered and the two levels mentioned earlier will be combined.
...
A summary of the variable as well as the Prosperity API confirm that `EmploymentStatusDuration` is measured in months. The variable will be viewed in years for the following plots, as years are much easier to visualize and understand.
...
The initial plot shows that the data is right skewed and unimodal. Thre is a long tail with some values way beyond the 3rd quartile. 50% of borrowers have employment durations between two and five and a half years.
...
The main features that are going to be explored are the variables that provide favorable conditions for both buyers and lenders.

Investigating each of the above variables, escpecially those that might be correlated with favorable conditions, might lead to more insight into understanding the underlying data.

Several variables were adapted and reordered, while one new variable, `Rating`, was recreated entirely. `EmploymentDuration` right skewed with a long tail.`LoanAmount` was multimodel, with several peaks throughout the distribution.
...
```{r echo=FALSE, Rating_AvgCreditScore, fig.width=10}
