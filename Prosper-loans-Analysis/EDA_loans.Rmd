...
```{r, echo=TRUE, Create_CreditScoreAverage}
...
```{r, echo=TRUE, TermLength_Factored}
...
```{r, echo=TRUE, BorrowerState_Order}
...
```{r echo=TRUE, LoanStatus_Relevel}
...
```{r echo=TRUE, EmploymentStatus_relevel}
...
The initial plot shows that there may be a correlation between interest rate and credit score. However, it is hard to tell from the plot's current state. Because of the way credit scores were recorded by _Prosper_, the credit scores all lie along similiar x values. This causes overplotting issues as the plot forms buckets of data. By adding some horizontal jitter, a better distribution can be formed.
...
With the visualization changes, there still seems to be a slight downward trend through the data. As credit score increases, the interest rate seems to decrease. This is a relationship that was expected.
...
Similar to the credit score scattercplot, the data points pool into buckets. Because `Prosper Score` is a discrete rather than continuous variable, the results have a lot of overplotting.

This distribution will be replotted using better breaks, horizontal jitter, and a higher alpha level.
...
This plot still shows the small correlation between rating and loan amount from the bivariate section. Most of the high risk loans are for smaller loans less than $5,000. The higher loan amount have almost no high risk accounts and only "C" or higher ratings. The banding in the color further demonstrate the strong relationship between ratings and interest rate seen in the previous plot.
...
The initial coloring looks promising. There is a clear separation between the ratings are the credit score increases.  However, due to the method of how the credit scores were collected, horizontal jitter needs to be added to the plot to fix overplotting.
...
I believe that this plot perfectly captures the relationship between interest rate, credit score, and ratings. The lower, less risk accounts with high credit scores get the best ratings by _Propser_, and thus the lowest interest rates. As the credit score decreases, the average interest rate increases and the ratings begin to change. There is a nice progression of the ratings as both the interest rate increases and the credit score decreases.
