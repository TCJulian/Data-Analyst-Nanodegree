...
One of most important variables for both a borrower and a lender is the interest rate of the loan. The interest rate is ultimately set by _Prosper_ and is determined by a variety of variables.
...
The interest rate curve seems to be slightly right skewed, with most values falling between 17% and 20%. There is an abnormal spike  of interest rates occuring at 32%. This could be because of overlapping binwidths. Let's graph interest rate again, but with smaller binwidths to investigate:
...
Many lenders also look at a borrower's income when determining whether to fund a loan. The intuition is that a borrower with more disposable income should be more likely to pay their monthly payments on a loan. This makes them a less risky investment. 
...
It appears that there are quite a few "NA's" in this variable. There is also some values above ten. The _Prosper_ API doesn't provide any insight into the values; on the contrary, values should only go up to ten. NA's and values greater than 10 will be left out of the resulting plot.
...
The Prosper Score distribution appears normally distributed. Most values fall between 4 and 8, with the median value at 6. 
...
Most borrowers are employed in some manner. Very few report being unemployed, retired, or part-time. I imagine most borrowers just report being "Employed" and leave out the details of their type of employment.
...
There are several variables that can be explored further that could potentially predict the interest rate a borrower has. Obviously `Credit Score` will be used. `Income`, `Loan Amount`, `Employment Duration/Status`, and `Debt/Income Ratio` all might be correlated with interest rate as well, and will definitely appear in the bivariate analysis. `Rating` and `Prosper Score` are other interesting variable to explore, especially with some of the other supporting variables.

In this analysis, there were several variables that were adapted so they could be properly used. `Income`, `Term Length`, `Loan Status`, `Borrower State`, and `Employment Status` all had their levels added, combined, or reordered. One new variable, `Rating`, was recreated entirely using the two other rating variables.  
...
ggplot(loans, aes(x = CreditScoreAverage, y = BorrowerRate)) +
  ...
  labs(x = 'Average Credit Score', 
       y = 'Interest Rate',
       title = "Interest Rate by Credit Score")
```

Because of the way credit scores were recorded by _Prosper_, the credit scores all lie along similiar x values. This causes overplotting issues as the plot forms buckets of data. By adding some horizontal jitter, a better shape can be formed.
...
At first glance, it seems as if there might be a slight downward trend through the data. As credit score increases, the interest rate seems to decrease. This is a relationship that we might expect.
...
Since _Prosper_ score is a risk measurement of a borrower, I expect that there will be some sort of relationship between it and interest rate.

```{r echo=FALSE, warning=FALSE, InterestRate_ProsperScore}
ggplot(loans, aes(x = ProsperScore, y = BorrowerRate)) +
  geom_point(alpha = 0.1, 
...
```
As predicted, there is a moderate negative correlation between interest rate and _Prosper_ score. As the score goes up (or as risk goes down), interest rates tend to decrease as well. Scores near the middle appear to have the most variance in interest rate, with some individuals having rates at 10% on the low side and 35% on the high side.
...
ggplot(data = subset(loans, BorrowerState != ""),
       aes(x = reorder(BorrowerState, -BorrowerRate, FUN=median), y = BorrowerRate)) +
  geom_boxplot() +
...
It is surprising to find that employment status and duration don't have much of a relationship with interest rate. The only red flags are when someone reports being umemployed. The difference between working for 6 months and working for 30 years doesn't seem to have much of a relationship with interest rate. 

### Summary Bivariate Analysis

The bivariate section definitly shed some light into the relationships between interest rate and some of the other variables. Interest rate appears to be negatively correlated with several variables in the dataset. , though it does not account for a large portion of its variance, based on it's R^2 value. Surprisingly, interest rates did not fluctate much between different states, even though cost of living is different amongst each of them.
...
If borrower ratings and interest rate are truly tied closely together, there whould be a clear segmentation between the amount of borrowers at each rating.
...
This desnity plot visualization really shows how each rating corresponds with a specific range of interest rates. The highest rating "AA" peaks at the low end of the interest rates. The rating under that, "A", peaks at a little higher interest rate. As you move down the tier list, the interest rates peak at higher and higher interest rates, with the High Risk accounts peaking at the 32% mark.
